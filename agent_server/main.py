...
def _voc_block(results: list) -> str:
    ...
    # 매뉴얼 블록이 "답변은 이 내용으로 만드세요"라고 강하게 말하는 바람에, 모델이 매뉴얼에
    # 없으면 **VOC에 답이 있는데도** "매뉴얼에는 없는 내용입니다"로 끝냈다(#179).
    # 두 근거는 대등하다 — 어느 쪽에 있든 답이 있으면 그것으로 답한다.
    lines.append("**여기 답이 있으면 그것으로 답합니다.** 매뉴얼에 없다는 이유로 "
                 "`확인되지 않았습니다`라고 끝내지 마세요 — 매뉴얼과 과거 사례는 대등한 "
                 "근거이고, 문서화되지 않은 문제일수록 답은 여기에 있습니다.")
