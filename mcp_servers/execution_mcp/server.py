...
FREE_TOOLS = {
    "run_command": {
        "handler": run_command,
        "description": (
            "**표준 리눅스 명령을 그대로 실행한다**(pwd·ls·df·du·find·head·cat·stat·quota·"
            "nvidia-smi 등). 이런 기본 명령은 **일부러 툴로 등록해 두지 않았다** — 도구 목록에 "
            "없다고 못 하는 것이 아니라, 이 툴로 하라는 뜻이다. "
            "다른 툴(등록된 사내 커맨드)은 **묻는 것을 정확히 답할 때만** 쓴다. 이름이 비슷해 "
            "보인다는 이유로 고르지 말고, 조금이라도 다르면 이 툴로 알맞은 명령을 실행한다"
            "(예: 홈 '경로'를 물었는데 할당량 조회 툴을 부르지 않는다). "
            ...
            "셸을 거치지 않으므로 파이프·리다이렉션·와일드카드는 동작하지 않고, "
            "화면을 붙잡는 대화형 명령(편집기·페이저·top 류)은 쓸 수 없다. "
